...
- [ ] Alternância de idioma PT/EN — escopo bem maior que os outros itens (toca toda string do site, todas as páginas); tratar como esforço dedicado à parte, não bundlar com os itens menores. **Único item restante da Fase 3**, dividido em sub-etapas:
  ...
  - [ ] **3f** — `calendario.html`, `corrida.html`, `treino.html`, `sprint.html` (mais `js/resultsTable.js`), com o toggle passando a fazer parte do `js/layout.js` (cabeçalho compartilhado)
  - [ ] **3g** — `piloto.html`, `equipe.html`, `comparador.html`
  - [ ] **3h** — `paradas.html`, `voltas-rapidas.html`, `confiabilidade.html`

> **Nota de arquitetura i18n**: o toggle PT/EN só foi adicionado no topbar da home nesta etapa (3e) — `js/layout.js` (cabeçalho das páginas secundárias) só ganha o toggle quando essas páginas também forem traduzidas (3f), pra não mostrar um botão que não faz nada nas páginas ainda não traduzidas.
